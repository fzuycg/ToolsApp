...
// 日志文件保存目录
static const NSString* LogFilePath = @"/Documents/YCGLog/";
// 日志压缩包文件名
static NSString* ZipFileName = @"YCGLog.zip";
...
#pragma mark - 写入日志
    ...
    dispatch_async(dispatch_queue_create("writeLog", nil), ^{
        ...
        // 写入数据
        [self writeFile:filePath stringData:logStr];
...
- (void)clearExpiredLog{
    ...
    for (NSString* file in files) {
        ...
        if ([[file pathExtension] isEqualToString:@"txt"]) {
            ...
            NSDate* date = [self.dateFormatter dateFromString:fileName];
            if (date) {
                NSTimeInterval oldTime = [date timeIntervalSince1970];
                NSTimeInterval currTime = [[LogManager getCurrentDate] timeIntervalSince1970];
